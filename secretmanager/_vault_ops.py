...
def rotate_vault_key(hvac_client, transit_key):
    ...
    try:
        ...
        logger.debug(f"Current Key Version: '{current_key['data']['latest_version']}'")
        ...
        if response['data']['latest_version'] == current_key['data']['latest_version']:
            logger.warning(f"Vault key '{transit_key}' was not rotated. It is still at version {current_key['data']['latest_version']}.")
            return {"status": "failure"}
        else:
            logger.debug(f"Vault key '{transit_key}' rotated successfully to version {response['data']['latest_version']}.")
