...
logging.info("SecretManager initializing.")
sm = SecretManager(vault_url, service_account, role, ca_path=ca_cert)
logging.info("Reading Kubernetes secret/ciphertext.")
k8s_old_secret = sm.read_namespaced_secret(namespace, secret_name, secret_data_name)
logging.info("Decrypting ciphertext with current vault key.")
decrypted_data = sm.decrypt_data_with_vault(transit_key, k8s_old_secret)
logging.info("Rotating vault key.")
sm.rotate_vault_key(transit_key)
logging.info("Re-encrypting data with new vault key.")
ciphertext = sm.encrypt_data_with_vault(transit_key, decrypted_data)
logging.info("Updating Kubernetes secret with new ciphertext.")
